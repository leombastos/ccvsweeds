...
# fit model on ind study --------------------------------------------------

RunModelOneStudyFun <- function(mydata) {
  ...
  F.tidy <- tidy(summary(F.res))
  
  F.tidy <- F.tidy %>%
    mutate(cilo = estimate - 1.96 * std.error,
           cihi = estimate + 1.96 * std.error
           
    ) %>%
    dplyr::select(estimate, std.error, p.value, cilo, cihi)
...
RunModelNoModsFun <- function(mydata) {
  ...
  F.tidy <- F.tidy %>%
    ...
    dplyr::select(pval, avg, cilo, cihi, pavg, plo, phi)
...
RunModelModsFun <- function(mydata, mymod) {
...
RunModelModsContrastFun <- function(mydata, mymod) {
...
SubsamplelmerModsFun <- function(mydata, mymod) {
...
BootFirstFullModsFun <- function(mydata, mymod){
...
BootModsFun <- function(mydata, mymod){
  
  est_clim <- BootFirstFullModsFun(mydata, mymod)
  ...
  for (i in c(1:n)) {
    ...
    F.est <- SubsamplelmerModsFun(mydata, mymod)
...
## 2-level moderators
#
get95CI_2lvl_Fun <- function(myd, myci, mymod) {
  ...
  # Level 1 of modifier
  #~~~~~~~~~~~~~~~~~
  myd1 <- myd %>% filter(modlvl == lvls[1]) %>% dplyr::select(est)
  v1 <- nrow(myd1)  
  ...
  # Level 2 of modifier
  #~~~~~~~~~~~~~~~~~
  myd2 <- myd %>% filter(modlvl == lvls[2]) %>% dplyr::select(est)
  v2 <- nrow(myd2)  
...
## 3-level moderators
#
get95CI_3lvl_Fun <- function(myd, myci, mymod) {
  ...
  # Level 3 of modifier
  #~~~~~~~~~~~~~~~~~
  myd3 <- myd %>% filter(modlvl == lvls[3]) %>% dplyr::select(est)
  v3 <- nrow(myd3)  
...
# Run model WITH continuous mods -----------------------------------------------------


RunModelContModsFun <- function(mydata, mymod) {
